...
*/
    ...
    FileDialog {
        ...
        onAccepted: {
            ...
            if (fileUrl) {
                ...
                var fileHandle = Qt.resolvedUrl(file);
                var fileData = fileHandle.data;

                // 使用 TextStream 读取文件内容
                var textStream = new TextStream(fileData, TextStream.ReadOnly);
                if (!textStream.atEnd()) {
                    fileContent = textStream.readAll();
                    console.log("文件内容：" + fileContent); // 输出文件内容
                }
            }
        }
    }
    ...
    Item
    {
        ...
        {
            id:button_send
            text:controller.isLoading ? timer_loading.loadingText :"打开文件"
            anchors
            {
                bottom: parent.bottom
                right: parent.right
